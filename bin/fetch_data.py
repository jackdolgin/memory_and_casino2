...
def reformat_data(version):
    ...
    identifiers = {'worker_id': [], 'assignment_id': [], 'wid': [], 'ipaddress': [], 'bonus': []}
    ...
    try:
        ...
        raw_identifiers_path = 'data/raw/identifiers.csv'
        if not os.path.exists(raw_identifiers_path):
            idf.to_csv(raw_identifiers_path)
        else:
            old_idf = pd.read_csv(raw_identifiers_path, dtype={'bonus':'string'})
            old_idf = old_idf.set_index('wid')            
            combined_df = pd.concat([old_idf, idf])
            combined_df = combined_df.drop_duplicates()
            print(combined_df)
            combined_df.to_csv(raw_identifiers_path)
