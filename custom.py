...
def download_datafiles(codeversion, name):
    contents = {
        ...
        "ipaddress": lambda p: p.ipaddress,
    }
    ...
    for p in query:
        try:
            ...
            def insert_text_before_comma_after_newline(text, insert_text):
                lines = text.split("\n")
                for i, line in enumerate(lines):
                    parts = line.split(",", 1)
                    if len(parts) > 1:
                        lines[i] = f"{parts[0]}{insert_text},{parts[1]}"
                
                result = "\n".join(lines)
                return result

            text = contents[name](p)
            insert_text = f":{contents['ipaddress'](p)}"

            new_text = insert_text_before_comma_after_newline(text, insert_text)
...
def compute_bonus():
    ...
    try:
        ...
        bonus = user_data["questiondata"]["bonus"] # get bonus amount from questiondata
        # if bonus == ".25":
        #     user.bonus = "0.75"
        # elif bonus == ".75":
        #     user.bonus = "1.00"
